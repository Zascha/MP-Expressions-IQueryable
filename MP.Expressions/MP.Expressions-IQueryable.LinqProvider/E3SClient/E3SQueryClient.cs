...
namespace MP.Expressions_IQueryable.LinqProvider.E3SClient
{
    public class E3SQueryClient
    {
        ...
        public IEnumerable<T> SearchFTS<T>(IEnumerable<string> query, int start = 0, int limit = 0) where T : E3SEntity
        {
        ...
        public virtual IEnumerable SearchFTS(Type type, IEnumerable<string> query, int start = 0, int limit = 0)
        {
            HttpClient client = CreateClient();
            var requestGenerator = new FTSRequestGenerator(BaseAddress);
